...
class AqItem:
    ...
    def __init__(self, item_no, mfr, model, category, spec, accs):
        ...
        self.spec = spec
        self.accs = accs
        self.line1 = "ITEM {} - {}\n".format(self.item_no, self.category)
        self.line2 = ''
...
def main(xl_file, save_as, save_path):
...
def clean_data(items):
    ...
    punct_patt = r"NIKEC(,/)"
    by_who = r"NIKEC[/,]\s?((?:BY|by|By)\s((?:[^/\s]+(?:/[^/\s]+)*)+))"
        ...
        if item.spec is not None:
            if item.spec[:5] == "NIKEC":
                ...
                if punct == ',':
                    print('comma')
                    punct += ' '
                item.line2 = 'NIKEC' + punct + \
                              re.search(by_who,item.spec).group(1).title() + \
                              '\n'
            ...
            if item.model[:5] == "NIKEC":
                ...
                if punct == ',':
                    punct += ' '
                item.line2 = 'NIKEC' + punct + 'By ' + \
                              re.search(by_who, item.model).group(1).title() + \
                             '\n'
                item.spec = nikec_string
                item.accs = []
